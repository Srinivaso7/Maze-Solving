...
class MazeGenerator:
    ...
    def carve_path(self, x, y, step_callback=None):
        self.maze[x][y] = 'EMPTY'
        ...
        for dx, dy in directions:
            ...
            if self.is_valid(nx, ny):
                ...
                if step_callback:
                    step_callback()
                self.carve_path(nx, ny, step_callback)

    def generate_dfs_maze(self, step_callback=None):
        # Start from the top-left corner
        self.carve_path(0, 0, step_callback)
        self.start_node = (0, 0)
    ...
    def generate_and_draw_maze(self):
        ...
        def step_callback():
            self.draw_maze()
            self.root.update()
            time.sleep(0.0001)  # Adjust the delay between steps if needed

        self.generate_dfs_maze(step_callback=step_callback)
